...
namespace HTWDiscordBot.Services.HTW
{
    public class RoleService
    {
        ...
        private const string activeHackerText = "Reagiere unten, um benachrichtigt zu werden, wenn eine neue Aufgabe verfügbar ist.";
        ...
        public async Task InitializeAsync()
        {
            ...
            SocketTextChannel? textChannel = await client.GetChannelAsync(configService.Config.RolesChannelID) as SocketTextChannel;
        ...
        private async Task Client_ReactionAdded(Cacheable<IUserMessage, ulong> cachedMessage, Cacheable<IMessageChannel, ulong> cachedChannel, SocketReaction reaction)
        {
            ...
            ulong roleID = messageID == configService.Config.ActiveHackerMessageID ? configService.Config.ActiveHackerRoleID : configService.Config.CTFRoleID;

            SocketRole? role = channel?.Guild.GetRole(roleID);
            if (role == null)
            {
                loggingService.Log(new(LogSeverity.Error, "RoleService", $"Rolle {roleID} konnte nicht gefunden werden!"));
                return;
            }
        ...
        private async Task Client_ReactionRemoved(Cacheable<IUserMessage, ulong> cachedMessage, Cacheable<IMessageChannel, ulong> cachedChannel, SocketReaction reaction)
        {
            SocketTextChannel? channel = cachedChannel.GetOrDownloadAsync().Result as SocketTextChannel;
